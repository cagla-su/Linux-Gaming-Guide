...
- **Step 6 - For Turkish Players Only**: For Turkish keyboard layout, you should add an environment variable after installing the client. Otherwise, you will be able to see the special characters like `ğ,ş,İ,ı` texted by other players on your screen but when you try to use the letters, you will see `g,s,I,i` as the output.
...
`~/Games/gameforge-client/drive_c/your-games-location/metin2/en-GB/bgm`
    - The `en-GB` folder's name might be something different if you are playing on another servers such as Türkiye (`tr-TR`) servers.

...
- Linux'ta **Windows oyunlarının birçoğunu** oynayabilirsiniz. Fakat, bu **Linux ile doğrudan uyumlu hiçbir oyunun mevcut olmadığı anlamına gelmez**. Bazı **Linux ile doğrudan uyumlu oyunlar** şunlardır:
  - Minecraft, Terraria, Stardew Valley, Euro Truck Simulator 2 vb.
...
- Windows oyunları Linux'ta çalışabilmek için bazı **temel araçlara** ihtiyaç duyarlar ve bu araçlar şunlardır:
	- [WINE](https://www.winehq.org/) / [Proton](https://github.com/ValveSoftware/Proton) **-** WINE, **Windows kütüphane dosyalarını Linux kütüphane dosyalarına** aktaran bir uyumluluk katmanıdır. Windows'taki **uyumluluk modu** ile aynı şekilde çalışır.
 		- Proton, WINE'ın Valve tarafından **çatallanmış** halidir ve **özel olarak Steam oyunları için tasarlanmıştır**.
	- [DXVK](https://github.com/doitsujin/dxvk) **-** DXVK, Windows oyunlarını Linux'ta çalıştırmak için **DirectX 8-11** çağrılarını **Vulkan'a** çevirir. DXVK WINE'a **dahil edilmemiştir** fakat yine de **DXVK'i elle kurmak zorunda değilsiniz** çünkü **Linux'ta oyun oynama yazılımlarının** ve **özel WINE/Proton yapılarının birçoğu** kullanıcılar için DXVK'i dahil eder.
		- **Benzer amaçlar** için kullanılan **başka araçlar** da vardır ve o araçlar şunlardır:
 			- [VKD3D](https://gitlab.winehq.org/wine/vkd3d) **-** VKD3D, DXVK'in henüz yapamadığı **DirectX 12** çağrılarını **Vulkan'a** çevirme işlemini yapar. Oynamaya çalıştığınız oyun bir **DirectX 12 oyunuysa** VKD3D **varsayılan olarak devreye girer**. Tıpkı DXVK gibi, VKD3D de **Linux'ta oyun oynama yazılımlarının** ve **özel WINE/Proton yapılarının birçoğunda kendiliğinden dahildir**.
			- [WINED3D](https://gitlab.winehq.org/wine/wine/-/tree/master/dlls/wined3d) **-** WINED3D **DirectX 8-11** çağrılarını **Vulkan yerine OpenGL'e** çevirir. Sisteminizde **Vulkan sürücüleri yüklü değilse** WINED3D **varsayılan olarak devreye girer**. Fakat DirectX çağrıları OpenGL'e çevrileceği için **düşük performans** almanız olasıdır çünkü OpenGL **artık geliştirilmiyor**. DXVK ve VKD3D'nin **aksine**, WINED3D WINE'a **dahildir**.
```
...
```
> [!NOTE]
> - WINE'ı **elle yüklemenize gerek yoktur** çünkü WINE siz Lutris gibi bir oyun oynama yazılımı yüklediğinizde **kendiliğinden yüklenir**.
> [!WARNING]
...
- Bazı çekirdek seviyesinde çalışan anti-hile yazılımları şunlardır:
	...
	- **XIGNCODE3**
	- **nProtect GameGuard**
- **Easy Anti-Cheat** ve **BattlEye'ın Linux ile uyumlu sürümleri mevcuttur**. Bu yüzden, bu anti-hile yazılımlarını kullanan bazı oyun geliştiricileri **WINE/Proton için özel olarak Linux uyumluluğunu etkinleştirebilirler**.
	- Ancak, Fortnite ve Apex Legends gibi oyunların geliştiricileri kendi anti-hile yapılandırmalarında **Linux desteğini etkinleştirmeyebilirler**. Bu yüzden **bu oyunlar Linux'ta çalışamazlar**.
...
- **VANGUARD** haricinde, yukarıda bahsedilen çekirdek seviyesinde çalışan anti-hile yazılımlarını kullanan oyunların **oynanabilirlikleri hakkında kesin bir ifadede bulunamayız**.
	- Durum VANGUARD için farklıdır çünkü VANGUARD yalnızca Riot Games'in oyunlarında kullanılır, ve Riot Games Linux'ta oyun oynayanları **desteklemeye niyetli bir şirket değildir**.
# Başlangıç
## Vulkan Sürücülerini Yükleme
> [!NOTE]
> - Eğer **NVIDIA** kullanıyorsanız, lütfen [Lutris'in topluluk rehberini](https://github.com/lutris/docs/blob/master/InstallingDrivers.md) takip edin.
> - Eğer **Intel/AMD** kullanıyorsanız, Vulkan sürücüleri siz **Steam** gibi bir **oyun oynama yazılımı** yüklediğinizde **kendiliğinden yüklenir**.
## Oyun Oynama Yazılımı Yükleme
- Linux'ta mevcut olan bazı bilinen oyun oynama yazılımları şunlardır:
	- [Steam](https://store.steampowered.com/)
		- [Heroic Games Launcher](https://heroicgameslauncher.com/) **-** Heroic, **Epic Games, GOG ve Prime Gaming** platformlarındaki oyunları oynamak için tasarlanmış oldukça yararlı bir oyun oynama yazılımdır.
	- [Lutris](https://lutris.net/) **-** Lutris **Windows oyunlarını oynamak için** tasarlanmış, özellik bakımından zengin olan bir oyun oynama yazılımıdır. Lutris aracılığıyla **Epic Games, Battle.net, Ubisoft vb.** gibi oyun oynama yazılımı platformlarını **doğrudan** yükleyebilirsiniz ama aynı zamanda herhangi bir platforma ait olmayan oyunları ve uygulamaları da yükleyip oynayabilirsiniz.
		- Lutris, **büyük miktarda** oyun ve yazılım için **topluluk tarafından oluşturulan yükleme ekranı** da sunar.
		- **DVD oyunları** da dahil olmak üzere, eğer elinizde kurulum dosyanız varsa, oyunlarınızı **elle** de kurabilirsiniz. 
	- [AAGL](https://github.com/an-anime-team/an-anime-game-launcher) **-** AAGL (An Anime Game Launcher) **Genshin Impact** oynamak için tasarlanmış, özellik bakımından zengin bir oyun oynama yazılımıdır. Ayrıca **telemetriyi devre dışı bırakır** ve güncelleme sonrası oyun bozulmasın diye **çeşitli iyileştirmelerle yamalar uygular**.
	- [ProtonPlus](https://protonplus.vysp3r.com/) **-** **Daha iyi uyumluluk ve performans** için **özel WINE/Proton yapılarını** ProtonPlus aracılığıyla yükleyebilirsiniz. Özel WINE/Proton yapıları Lutris, Steam ve Heroic tarafından **desteklenmektedir**.
	- [Sober](https://sober.vinegarhq.org) **-** Sober, **Roblox'un** Android versiyonunu Linux'ta oynamaya olanak tanıyan bir oyun oynama yazılımıdır. Geçmişte **VinegarHQ** aracılığıyla Roblox'un Windows versiyonunu oynamak mümkündü. Fakat şu anda **bu destek anti-hile yapılandırması yüzünden kesildi**. Bu yüzden aynı geliştirici ekibi **Sober'ı** geliştirdi.
	- [Prism Launcher](https://prismlauncher.org) **-** Prism Launcher, **Minecraft** oynamak için tasarlanmış, özellik bakımından zengin bir oyun oynama yazılımıdır.
 		- İlginizi çekerse, Minecraft'ı yapılandırmak ve performans bakımından en iyi hâle getirmek hakkında yazdığım bir [Minecraft rehberim](https://github.com/cagla-su/Linux-Gaming-Guide/blob/main/T%C3%BCrk%C3%A7e%20%C3%87eviri/Oyunlara%20%C3%96zel%20Rehberler/Minecraft-tr.md) mevcuttur.
	- [Waydroid](https://waydro.id/) **-** Waydroid Linux'ta Android'i çalıştırmanıza olanak sağlayan **konteyner tabanlı bir uyumluluk katmanıdır**. Yani, Linux'ta Android oyunlarını Waydroid aracılığıyla oynayabilirsiniz.
 		- İlginizi çekerse, aklınızdaki sorularınız, yükleme ve yapılandırma adımları için yazdığım bir [Waydroid rehberim](https://github.com/cagla-su/Waydroid-Guide/blob/main/Waydroid-Rehberi.md) mevcuttur.

> [!TIP]
> - Bu yazılımları **kendi paket yöneticinizden** veya [Flatpak](https://flatpak.org)'ten yükleyebilirsiniz.
> - Steam oyunlarınızın Linux'ta oynanabilir olup olmadığını görmek için [ProtonDB](https://www.protondb.com)'ye bakabilirsiniz.
...
>	- Oyununuz için herhangi bir değerlendirme yapılmışsa **performans açısından en iyi hâle getirme tavsiyeleri** alabilirsiniz. İsterseniz, dilediğiniz Steam oyunu hakkında kendi değerlendirmelerinizi de yazabilirsiniz.
...
- Oyunları harici **AMD** ekran kartıyla çalıştırmak için aşağıdaki komutu kullanın:
```
DRI_PRIME=1
```
 - Oyunları **NVIDIA** ekran kartıyla çalıştırmak için aşağıdaki komutu kullanın:
```
...
- Bu örnek .desktop dosya içeriği NVIDIA ekran kartıyla Prism Launcher'ı çalıştırmaktadır. Odaklanmanız gereken satır `Exec` ile başlayan satırdır:
```
[Desktop Entry]
Version=1.0
Type=Application
...
```
> [!NOTE]
> Eğer **ek olarak** diğer başlatma seçeneklerini de kullanacaksanız ve oyunları harici ekran kartınızla çalıştıracaksanız, tüm komutların örnekte olduğu gibi `%command%` **yer tutucusundan önce** yazıldığından emin olun:
```
...
- Paket yöneticinizi kullanarak `gparted` paketini yükleyin ve harici diskinizin **fiziksel olarak bağlıyken yazılım tarafında bağlı olmadığından emin olarak** sıradaki adımları takip edin:
	- `GParted'ı çalıştırın` **-** `sağ üstten harici diskinizi seçin` **-** `tüm dizinleri silin` **-** `Add`
 	- **Sonra -** `File system: ext4 (veya dilerseniz xfs)` **-** `Label: istediğiniz herhangi bir başlık` **-** `Add`
...
### Lossless Scaling
[Lossless Scaling](https://store.steampowered.com/app/993090/Lossless_Scaling/) sayesinde oyunlarınızı **görsel kalite kaybı olmadan daha düşük çözünürlüklerde** oynayabilirsiniz. Ancak, uygulamayı kullanabilmek için uygulamayı Steam'den **satın almalısınız**.
- Lossless Scaling'i satın aldıktan sonra onu Linux'ta kullanabilmek için [lsfg-vk](https://github.com/PancakeTAS/lsfg-vk)'i kurmalısınız.
...
- **Arch Linux ve çeşitleri:** [CachyOS Çekirdeği](https://github.com/CachyOS/linux-cachyos)
- **Fedora Linux ve çeşitleri:** [Fedora İçin CachyOS Çekirdeği](https://github.com/CachyOS/copr-linux-cachyos)
- **Debian/Ubuntu Linux ve çeşitleri:** [XanMod Çekirdeği](https://xanmod.org/)
## Gamemode
...
> [!NOTE]
> - Genellikle, Lutris gibi oyun oynama yazılımları bunu **kendiliğinden etkinleştirirler**.
> - Fakat Steam'de oyununuzun başlatma seçeneklerine örnekte olduğu gibi **elle bir komut eklemelisiniz**:
```
gamemoderun %command%
```
...
- `WINEDEBUG=-all` **-** WINE'ın tüm hata ayıklama çıktılarının üretimini durdurur**. - *Performansa etkisi düşüktür*
...
## Kompozitörü Devre Dışı Bırakma - YALNIZCA X11 İÇİN GEÇERLİDİR!!!
> [!WARNING]
>  Eğer Wayland kullanıyorsanız, kompozitörü devre dışı bırakmak **mümkün değildir**. Bu adımlar yalnızca X11 için geçerlidir.
...
- Bu adım [Linux Gaming Wiki'de](https://linux-gaming.kwindu.eu/index.php?title=Compositor_(X11)) açıklanmıştır.
# Kapanış
Bu rehber Linux'ta oyun oynama hakkındaydı. Okuduğunuz için teşekkürler. Umarım bu rehber sisteminizi Linux'ta oyun oynamaya hazır hâle getirme konusunda yardımcı olmuştur. İyi oyunlar!
